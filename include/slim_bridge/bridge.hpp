...
#ifndef INCLUDE_SLIM_BRIDGE_BRIDGE_HPP_
#define INCLUDE_SLIM_BRIDGE_BRIDGE_HPP_
...
void read_yaml(const YAML::Node & data,
    ...
    if (data["latch"])
        options.latch = data["latch"].as<bool>();
    else
        options.latch = true;

    options.qos = reliable_qos;
    options.qos.reliability(
        reliability_table[data["reliability"].as<std::string>()]);
    options.qos.durability(
        durability_table[data["durability"].as<std::string>()]);
...
#endif  // INCLUDE_SLIM_BRIDGE_BRIDGE_HPP_
